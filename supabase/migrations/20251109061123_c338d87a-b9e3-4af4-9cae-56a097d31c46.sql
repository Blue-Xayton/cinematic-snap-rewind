-- Create app_role enum for collaboration if it doesn't exist
DO $$ BEGIN
...
DROP POLICY IF EXISTS "Anyone can view templates" ON public.video_templates;
...
DROP POLICY IF EXISTS "Job owners can manage shares" ON public.shared_jobs;
...
DROP POLICY IF EXISTS "Users can view shares with them" ON public.shared_jobs;
...
INSERT INTO public.video_templates (name, description, mood, track, target_duration, default_transitions, thumbnail_url, is_premium)
SELECT * FROM (VALUES
  ('Cinematic Journey', 'Epic and dramatic video with smooth transitions', 'cinematic', 'track1', 30, '["fade", "zoom", "fade"]'::jsonb, 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=400', false),
  ('Upbeat Memories', 'Fun and energetic video perfect for celebrations', 'upbeat', 'track2', 30, '["slide", "zoom", "slide"]'::jsonb, 'https://images.unsplash.com/photo-1492684223066-81342ee5ff30?w=400', false),
  ('Chill Vibes', 'Relaxed and smooth flow for peaceful moments', 'chill', 'track3', 45, '["fade", "fade", "fade"]'::jsonb, 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400', false),
  ('Adventure Reel', 'Fast-paced excitement for outdoor adventures', 'epic', 'track1', 30, '["zoom", "slide", "zoom"]'::jsonb, 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?w=400', false),
  ('Romantic Story', 'Gentle and emotional journey for special moments', 'romantic', 'track4', 60, '["fade", "zoom", "fade"]'::jsonb, 'https://images.unsplash.com/photo-1518621736915-f3b1c41bfd00?w=400', true)
) AS v(name, description, mood, track, target_duration, default_transitions, thumbnail_url, is_premium)
WHERE NOT EXISTS (SELECT 1 FROM public.video_templates LIMIT 1);

-- Add triggers for updated_at
DROP TRIGGER IF EXISTS update_video_templates_updated_at ON public.video_templates;
CREATE TRIGGER update_video_templates_updated_at
BEFORE UPDATE ON public.video_templates
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at();

DROP TRIGGER IF EXISTS update_user_tutorial_progress_updated_at ON public.user_tutorial_progress;
